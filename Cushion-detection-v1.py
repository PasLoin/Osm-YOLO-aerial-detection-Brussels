### Detection of traffic calming = cushion using Urbis Orhto Photos 2023 and custom model ###
### This is a working P.O.C that can generate false positive ###
### Do no use to add blindly without checking street level imagery ###

# To DO :
## - Clean the code
## - Add how to for creating a model
## - removing false positive with re-training models

# Detection tested with CPU on colab and Kaggle
...
import math
!wget https://raw.githubusercontent.com/PasLoin/Osm-YOLO-aerial-detection-Brussels/main/models/cushion/cushion-v1.pt
# Load your trained model

# Load your trained model
model = YOLO('cushion-v1.pt')
...
large_bbox_wgs84 = [4.332261,50.855986,4.344041,50.861634]  # Change to your needs
...
def create_josm_xml(detections):
    ...
    for i, detection in enumerate(detections):
        ...
        tag = SubElement(node, 'tag', k="traffic_calming", v="cushion")
...
def fetch_existing_osm_data(bbox):
    ...
    [out:json];
    (
      node["traffic_calming"="cushion"]({bbox[1]},{bbox[0]},{bbox[3]},{bbox[2]});
    );
    out body;
    >;
    out skel qt;
...
# Function to Process Detected Cushions and Create JOSM XML
def process_detections(num_tiles_x, num_tiles_y, bbox_data, tiles, batch_size=100, threshold_meters=10):
                    ...
                    if min_distance < threshold_meters:
                        all_detections.append({
                            ...
                            "properties": {
                                ...
                                "task_description": "Add traffic calming cushion",
                                "task_instructions": "Verify the presence of a traffic calming cushion and add it if missing.",
    ...
    create_geojson(detections_more_than_10m, 'detections_more_than_10m.geojson')
